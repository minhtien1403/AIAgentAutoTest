...
final class TaskStatusBadgeView: UIView {
    enum DisplayStyle {
        case row
        case tag
    }
    ...
    func configure(taskStatus: TaskStatus, accessibilityId: String, style: DisplayStyle = .row) {
        label.text = taskStatus.displayName

        switch taskStatus {
        case .todo:
            backgroundColor = UIColor.systemGray.withAlphaComponent(0.22)
        ...
        accessibilityLabel = String(localized: "Status \(taskStatus.displayName)")
        accessibilityIdentifier = accessibilityId
        accessibilityValue = taskStatus.displayName
